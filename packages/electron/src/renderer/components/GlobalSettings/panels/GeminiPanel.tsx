    ...
    <div className="provider-panel gemini-provider-panel flex flex-col">
      <div className="provider-panel-header mb-6 pb-4 border-b border-[var(--nim-border)]">
        <h3 className="provider-panel-title text-xl font-semibold leading-tight mb-2 text-[var(--nim-text)] flex items-center gap-2">
          {t('gemini.title', 'Gemini')}
          <AlphaBadge size="sm" tooltip={SETTINGS_ALPHA_TOOLTIP} />
        </h3>
        <p className="provider-panel-description text-sm leading-relaxed text-[var(--nim-text-muted)]">
          {t('gemini.description', "Google's Gemini models, run through the language server that ships inside Antigravity. Uses the account you are already signed into there.")}
        </p>
      </div>

      <div className="provider-panel-section py-4 mb-4 border-b border-[var(--nim-border)]">
        <h4 className="provider-panel-section-title text-base font-semibold mb-3 text-[var(--nim-text)]">
          {t('gemini.antigravity', 'Antigravity')}
        </h4>

        {status === 'checking' && (
          <p className="text-[13px] text-[var(--nim-text-muted)]">{t('gemini.checking', 'Looking for Antigravity...')}</p>
        )}
        ...
        {status === 'installed' && (
          <div>
            <div className="flex items-center gap-2">
              ...
              <span className="text-[13px] text-[var(--nim-text)]">{t('gemini.installed', 'Installed')}</span>
            </div>
            {installPath && (
        ...
        {status === 'not-installed' && (
          <div>
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="text-[13px] text-[var(--nim-text)]">{t('gemini.not_found', 'Not found')}</span>
            </div>
            <p className="text-[13px] text-[var(--nim-text-muted)] mb-3 leading-relaxed">
              {t('gemini.not_installed_desc', 'Install Antigravity and sign in once. Nimbalyst does not install it for you, and it does not need to stay open — only the language server inside it is used.')}
            </p>
            <button
        ...
        <p className="text-[13px] text-[var(--nim-text-muted)] mt-3 leading-relaxed">
          ...
          >
            {t('gemini.docs_link', 'antigravity.google')}
          </a>
          {' '}{t('gemini.docs_suffix', 'for downloads and sign-in.')}
        </p>
      </div>

      <SettingsToggle
        variant="enable"
        name={t('gemini.enable', 'Enable Gemini')}
      ...
      {status === 'installed' && (
        <p className="text-[13px] text-[var(--nim-text-muted)] mt-2 leading-relaxed">
          {t('gemini.default_on_note', 'On by default because Antigravity is installed. Turning it off here is remembered.')}
        </p>
      )}
      ...
      {config.enabled && (
        <>
          <div className="provider-panel-section py-4 mb-4 border-b border-[var(--nim-border)]">
            <h4 className="provider-panel-section-title text-base font-semibold mb-3 text-[var(--nim-text)]">{t('gemini.authentication', 'Authentication')}</h4>
            <p className="text-[13px] text-[var(--nim-text-muted)] mb-3 leading-relaxed">
              {t('gemini.auth_desc1', 'Sign in inside Antigravity. The language server uses that login, so there is nothing to enter here.')}
            </p>
            <p className="text-[13px] text-[var(--nim-text-muted)]">
              {t('gemini.auth_desc2', 'No API key is required, and Nimbalyst never reads one from your environment.')}
            </p>
          </div>

          <div className="provider-panel-section py-4 mb-4 border-b border-[var(--nim-border)]">
            <h4 className="provider-panel-section-title text-base font-semibold mb-3 text-[var(--nim-text)]">{t('gemini.file_tracking', 'File tracking')}</h4>
            <p className="text-[13px] text-[var(--nim-text-muted)] leading-relaxed">
              {t('gemini.file_tracking_desc', "Nimbalyst performs this agent's writes itself, so every edit records the file's exact contents from before it changed and diff review is precise. Deletes and renames go through shell commands instead; Nimbalyst watches the project folder to catch those, so one can occasionally be attributed to the wrong turn.")}
            </p>
          </div>

          <div className="provider-panel-section py-4 mb-4 border-b border-[var(--nim-border)] last:border-b-0 last:mb-0 last:pb-0">
            <h4 className="provider-panel-section-title text-base font-semibold mb-3 text-[var(--nim-text)]">{t('gemini.context_usage', 'Context usage')}</h4>
            <p className="text-[13px] text-[var(--nim-text-muted)] leading-relaxed">
              {t('gemini.context_usage_desc', 'Antigravity reports no token counts, so Nimbalyst shows no context meter for Gemini rather than a made-up one. Remaining quota is shown in the usage indicator instead.')}
            </p>
          </div>
        </>
      )}
