    ...
    <div className="provider-panel flex flex-col">
      ...
      <div className="provider-panel-section">
          ...
          <p className="provider-panel-hint">
            {t('advanced.app_mode_desc', 'Choose between a simplified experience or full developer features for this project.')}
          </p>
          ...
          <div className="mode-selection flex flex-row gap-4 mt-3">
            ...
            >
              ...
              <div className="p-4 w-full flex flex-col items-center text-center">
                <div className="flex flex-col items-center gap-2 mb-2">
                  ...
                  <span className="text-base font-semibold text-nim">{t('advanced.standard_mode_short', 'Standard Mode')}</span>
                </div>
                <p className="m-0 text-[13px] leading-snug text-nim-muted">
                  {t('advanced.standard_mode_short_desc', 'Simplified interface focused on writing, editing, and AI assistance')}
                </p>
              </div>
            </label>
            ...
            >
              ...
              <div className="p-4 w-full flex flex-col items-center text-center">
                ...
                <p className="m-0 text-[13px] leading-snug text-nim-muted">
                  {t('advanced.dev_mode_short_desc', 'Full development environment with git worktrees, terminal access, development specific features')}
                </p>
              </div>
            </label>
          </div>
      ...
      {showFeaturesMenu && (
        <div className="provider-panel-section py-4 mb-4 border-b border-[var(--nim-border)] last:border-b-0 last:mb-0 last:pb-0">
          <h4 className="provider-panel-section-title text-base font-semibold mb-3 text-[var(--nim-text)]">
            {t('advanced.feature_availability', 'Feature Availability')}
          </h4>
          <p className="text-sm leading-relaxed text-[var(--nim-text-muted)] mb-4">
            {t('advanced.feature_availability_desc', 'See which features are available based on your current mode settings.')}
          </p>
              ...
              <label className="setting-label">
                ...
                <div className="setting-text">
                  ...
                  <span className="setting-description">
                    {t('advanced.all_dev_features_desc', 'Enable or disable all developer features at once')}
                  </span>
                </div>
              </label>
            </div>
                  ...
                  <label className="setting-label">
                    ...
                    <div className="setting-text">
                      <span className="setting-name flex items-center gap-2">
                        ...
                        >
                          {isAvailable ? t('advanced.available', 'Available') : t('advanced.hidden', 'Hidden')}
                        </span>
                      </span>
          ...
          <p className="text-xs text-[var(--nim-text-faint)] mt-3">
            {t('advanced.dev_mode_status', { status: developerMode ? t('advanced.dev_mode_on', 'ON') : t('advanced.dev_mode_off', 'OFF'), defaultValue: `Developer mode: ${developerMode ? 'ON' : 'OFF'}` })}
          </p>
        </div>
      )}
      ...
      <div className="provider-panel-section py-4 mb-4 border-b border-[var(--nim-border)] last:border-b-0 last:mb-0 last:pb-0">
        ...
        <p className="text-sm leading-relaxed text-[var(--nim-text-muted)] mb-4">
          {t('advanced.release_channel_desc2', "Choose which release stream Nimbalyst pulls auto-updates from. Switching channels keeps your installed version until a newer release is available. Installing an older version requires a manual download and install. Alpha and beta features are configured separately on each feature's settings page.")}
        </p>

        <div className="setting-item py-3">
          <div className="setting-text flex flex-col gap-0.5">
            <span className="setting-name text-sm font-medium text-[var(--nim-text)]">{t('advanced.update_channel', 'Update Channel')}</span>
            <span className="setting-description text-xs leading-relaxed text-[var(--nim-text-muted)]">
              <strong>{t('advanced.channel_option_stable', 'Stable')}:</strong> {t('advanced.channel_stable_desc', 'Production-ready releases (recommended for most users).')}<br/>
              <strong>{t('advanced.channel_option_alpha', 'Alpha')}:</strong> {t('advanced.channel_alpha_desc', 'Frequent, rough developer releases. Expect bugs and breaking changes between updates.')}
            </span>
          </div>
          <select
          ...
          >
            <option value="stable">{t('advanced.channel_option_stable', 'Stable')}</option>
            <option value="alpha">{t('advanced.channel_option_alpha', 'Alpha (Developer Releases)')}</option>
          </select>
        </div>
        ...
        {releaseChannel === 'alpha' && (
          <div className="mt-3 flex items-start gap-2 p-3 rounded border border-[var(--nim-warning)]/30 bg-[var(--nim-warning)]/10">
            ...
            <p className="m-0 text-[13px] text-[var(--nim-text)] leading-snug">
              {t('advanced.alpha_warning', 'The alpha channel ships rough developer releases that may be unstable or contain unfinished work. Switch back to Stable if you encounter problems.')}
            </p>
          </div>
        )}
      </div>
      ...
      <div className="provider-panel-section py-4 mb-4 border-b border-[var(--nim-border)] last:border-b-0 last:mb-0 last:pb-0">
        ...
        <SettingsToggle
          ...
          name={t('advanced.send_usage_data', 'Send Anonymous Usage Data')}
          description={t('advanced.send_usage_data_desc', 'Help improve Nimbalyst by sending anonymous usage data. No prompts or personal info collected.')}
        />

        <SettingsToggle
          checked={spellcheckEnabled}
          onChange={(checked) => updateSettings({ spellcheckEnabled: checked })}
          name={t('advanced.spellcheck', 'Spellcheck')}
          description={t('advanced.spellcheck_desc', 'Enable the system spellchecker in editors and text inputs.')}
        />

        <SettingsToggle
          checked={showDirectChatProviders}
          onChange={(checked) => updateSettings({ showDirectChatProviders: checked })}
          name={t('advanced.show_chat_providers', 'Show Chat Providers')}
          description={t('advanced.show_chat_providers_desc', 'Show Claude Chat, OpenAI, and LM Studio in provider settings and new-session model selection.')}
        />

        <SettingsToggle
          checked={walkthroughsEnabled}
          onChange={(checked) => updateSettings({ walkthroughsEnabled: checked })}
          name={t('advanced.show_feature_guides', 'Show Feature Guides')}
          description={t('advanced.show_feature_guides_desc', { viewed: walkthroughsTotalCount > 0 ? t('advanced.show_feature_guides_viewed', { viewed: walkthroughsViewedCount, total: walkthroughsTotalCount, defaultValue: ` (${walkthroughsViewedCount}/${walkthroughsTotalCount} viewed)` }) : '', defaultValue: `Walkthrough guides for new features and tips.${walkthroughsTotalCount > 0 ? ` (${walkthroughsViewedCount}/${walkthroughsTotalCount} viewed)` : ''}` })}
        />

        {walkthroughsViewedCount > 0 && (
          <div className="py-1 pl-7">
            <button onClick={() => resetWalkthroughs()} className="nim-btn-secondary text-xs">
              {t('advanced.reset_all_guides', 'Reset All Guides')}
            </button>
          </div>
        )}
      </div>
      ...
      <div className="provider-panel-section py-4 mb-4 border-b border-[var(--nim-border)] last:border-b-0 last:mb-0 last:pb-0" data-testid="tracker-automation-section">
        ...
        <SettingsToggle
          ...
          name={t('advanced.link_commits', 'Link Commits to Tracker Items')}
          description={t('advanced.link_commits_desc', 'Link git commits to tracker items via session relationships and issue key parsing (e.g. NIM-123 in commit messages).')}
        />
        ...
        {trackerAutomation.enabled && (
          <SettingsToggle
            ...
            name={t('advanced.close_on_fix', 'Close Items on Fixes/Closes/Resolves')}
            description={t('advanced.close_on_fix_desc', 'Change tracker item status to done when a commit message uses a closing keyword.')}
          />
        )}
      </div>
      ...
      <div className="provider-panel-section py-4 mb-4 border-b border-[var(--nim-border)] last:border-b-0 last:mb-0 last:pb-0">
        ...
        <DropdownRow
          ...
          name={t('advanced.external_editor2', 'External Editor')}
          description={t('advanced.external_editor2_desc', "Editor for the 'Open in...' context menu option.")}
          options={[
            { value: 'none', label: t('advanced.editor_none', 'None') },
            ...
            { value: 'custom', label: t('advanced.editor_custom', 'Custom...') },
          ]}
        />
        ...
        <SettingsToggle
          ...
          name={t('advanced.extension_dev_tools', 'Extension Dev Tools')}
          description={t('advanced.extension_dev_tools_desc', 'Enable MCP tools for building, installing, and hot-reloading extensions.')}
        />

        <DropdownRow
          value={maxHeapSizeMB}
          onChange={(val) => updateSettings({ maxHeapSizeMB: parseInt(val, 10) })}
          name={t('advanced.max_heap_size', 'Max Heap Size')}
          description={t('advanced.max_heap_size_desc', 'V8 memory limit. Increase if you get out-of-memory crashes. Requires restart.')}
          options={[
            { value: 2048, label: '2 GB' },
            { value: 4096, label: `4 GB${t('advanced.default_suffix', ' (Default)')}` },
        ...
        {process.platform === 'win32' && (
          <>
            <DropdownRow
              ...
              name={t('advanced.preferred_terminal_shell', 'Preferred Terminal Shell')}
              description={t('advanced.preferred_terminal_shell_desc', 'Choose which detected Windows shell new terminals should open with. Auto follows the built-in priority.')}
              options={terminalShellOptions}
            />

            <div className="setting-item py-2">
              <div className="setting-text flex flex-col gap-0 mb-2">
                <span className="setting-name text-sm font-medium text-[var(--nim-text)]">{t('advanced.detected_shells', 'Detected Terminal Shells')}</span>
                <span className="setting-description text-xs leading-snug text-[var(--nim-text-muted)]">
                  {t('advanced.detected_shells_desc', 'Current Windows shell discovery results used for terminal selection and restore.')}
                </span>
              </div>
              ...
              <div className="select-text p-2 rounded-md text-xs bg-[var(--nim-bg-tertiary)] border border-[var(--nim-border)] text-[var(--nim-text-muted)] font-mono">
                {availableTerminalShells.length === 0 ? (
                  <div>{t('advanced.no_shells_detected', 'No supported terminal shells detected.')}</div>
                ) : (
        ...
        <DropdownRow
          ...
          name={t('advanced.history_retention', 'History Retention')}
          description={t('advanced.history_retention_desc', 'Max age of file history snapshots before automatic cleanup.')}
          options={[
            { value: 7, label: `7 ${t('advanced.days', 'days')}` },
            { value: 14, label: `14 ${t('advanced.days', 'days')}` },
            { value: 30, label: `30 ${t('advanced.days', 'days')}${t('advanced.default_suffix', ' (Default)')}` },
            { value: 60, label: `60 ${t('advanced.days', 'days')}` },
            { value: 90, label: `90 ${t('advanced.days', 'days')}` },
            { value: 180, label: `180 ${t('advanced.days', 'days')}` },
            { value: 365, label: `1 ${t('advanced.year', 'year')}` },
          ]}
        />
        ...
        <DropdownRow
          ...
          name={t('advanced.max_snapshots', 'Max Snapshots Per File')}
          description={t('advanced.max_snapshots_desc', 'Oldest snapshots beyond this limit are deleted.')}
          options={[
            { value: 50, label: '50' },
            { value: 100, label: '100' },
            { value: 250, label: `250${t('advanced.default_suffix', ' (Default)')}` },
            { value: 500, label: '500' },
        ...
        <div className="setting-item py-2">
          <div className="setting-text flex flex-col gap-0 mb-2">
            <span className="setting-name text-sm font-medium text-[var(--nim-text)]">{t('advanced.custom_path_dirs', 'Custom PATH Directories')}</span>
            <span className="setting-description text-xs leading-snug text-[var(--nim-text-muted)]">
              {t('advanced.custom_path_dirs_desc', 'Additional directories for MCP server installation, CLI tool detection, and agent SDK operations.')}
            </span>
          </div>
          <textarea
          ...
          <div className="mt-1">
            ...
            >
              {showEnhancedPath ? t('advanced.hide_current_path', 'Hide current PATH') : t('advanced.show_current_path', 'Show current PATH')}
            </button>
...
function MultiProjectModeToggle() {
  ...
  const handleChange = async (next: boolean) => {
    if (!next && openProjects.length > 1) {
      const proceed = window.confirm(
        t('advanced.multi_project_confirm', { count: openProjects.length, defaultValue: `${openProjects.length} projects are open in the rail. Disable multi-project mode? The other projects will be closed (their unsaved work stays on disk).` })
      );
      if (!proceed) return;
  ...
  return (
    <SettingsToggle
      ...
      name={t('advanced.multi_project_mode', 'Multi-project Mode')}
      description={t('advanced.multi_project_mode_desc', 'Open multiple projects in a single window via a project rail. When off, each project opens in its own window.')}
    />
  );
}
...
function RestorePreviousProjectsToggle() {
  ...
  return (
    <SettingsToggle
      checked={enabled}
      onChange={setEnabled}
      name={t('advanced.restore_projects', "Restore last session's projects on launch")}
      description={
        isMultiProject
          ? t('advanced.restore_projects_desc_on', 'When on, the project rail rehydrates with every project that was open at last close. When off, only the project you pick from the launch screen opens.')
          : t('advanced.restore_projects_desc_off', 'Only takes effect when Multi-project Mode is enabled.')
      }
    />
  );
}
