  ...
  return (
    <div className="provider-panel flex flex-col">
      <div className="provider-panel-header mb-6 pb-4 border-b border-[var(--nim-border)]">
        <h3 className="provider-panel-title text-xl font-semibold leading-tight mb-2 text-[var(--nim-text)]">{t('lmstudio.title', 'LM Studio')}</h3>
        <p className="provider-panel-description text-sm leading-relaxed text-[var(--nim-text-muted)]">
          {t('lmstudio.description', 'Connect to local LLMs running in LM Studio on your machine. Start LM Studio and load a model before enabling.')}
        </p>
      </div>

      <SettingsToggle
        variant="enable"
        name={t('lmstudio.enable', 'Enable LM Studio')}
      ...
      {config.enabled && (
        <>
          <div className="provider-panel-section py-4 mb-4 border-b border-[var(--nim-border)] last:border-b-0 last:mb-0 last:pb-0">
            <h4 className="provider-panel-section-title text-base font-semibold mb-3 text-[var(--nim-text)]">{t('lmstudio.server_config', 'Server Configuration')}</h4>
          ...
          <div className="provider-panel-section py-4 mb-4 border-b border-[var(--nim-border)] last:border-b-0 last:mb-0 last:pb-0">
            ...
            {loading && (
              <div className="models-loading text-sm text-[var(--nim-text-muted)] py-2">{t('lmstudio.loading_models', 'Loading models from LM Studio...')}</div>
            )}

            {!loading && availableModels.length > 0 && (
              <div className="models-section">
                <div className="models-header flex items-center justify-between mb-3">
                  <span className="text-sm text-[var(--nim-text-muted)]">{t('lmstudio.detected_models', 'Detected models:')}</span>
            ...
            {!loading && availableModels.length === 0 && (
              <div className="models-loading text-sm text-[var(--nim-text-muted)] py-2">
                {t('lmstudio.no_models_found', 'No models found. Make sure LM Studio is running with a loaded model.')}
              </div>
            )}
            ...
            <div className="mt-4">
              <button
                ...
                disabled={loading}
              >
                {t('lmstudio.refresh_models', 'Refresh Models')}
              </button>
            </div>
          </div>
        </>
